...
if [ "${ENABLE_REMOTE_DATA_UPDATES:-false}" == "true" ] && [[ "$REPO_RAW_URL" =~ /(main|master)$ ]] && [ "${ALLOW_FLOATING_REMOTE:-false}" != "true" ]; then
    log "Updater" "WARN " "安全版拒绝从浮动分支拉取远程数据；请将 REPO_RAW_URL 固定到已审计 commit/tag"
...
# ==========================================================
if [ "${ENABLE_REMOTE_DATA_UPDATES:-false}" == "true" ]; then
NOW=$(date +%s)
LAST_UPDATE=0
...
if [ -n "$REGION_JSON_FILE" ] && [ -f "$REGION_JSON_FILE" ]; then
    ...
    log "Updater" "INFO " "安全版默认禁用远程数据热更新；保留本地 user_agents、keywords 与 region JSON"
fi

# ==========================================================
# [容灾校验] 外置供应链投毒防线与底层签名嗅探
# ==========================================================
if [ "${ENABLE_THIRD_PARTY_PROBE:-false}" == "true" ] && [ "${ALLOW_THIRD_PARTY_PROBE_DOWNLOAD:-false}" == "true" ]; then
...
elif [ "${ENABLE_THIRD_PARTY_PROBE:-false}" == "true" ]; then
    log "Updater" "WARN " "第三方探针已启用，但安全版拒绝自动下载；请人工审计并放置 core/ip_probe.sh"
else
    log "Updater" "INFO " "🛡️ 安全版默认禁用第三方探针自动更新"
fi
