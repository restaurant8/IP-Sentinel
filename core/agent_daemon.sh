...
if ! [[ "$CONTROL_SECRET" =~ ^[A-Fa-f0-9]{64,128}$ ]]; then
    echo "⛔ [Agent] CONTROL_SECRET 缺失或强度不足，拒绝启动远程控制端。"
    exit 1
fi
...
# [权限鉴权] 提取高熵 CONTROL_SECRET 作为 PSK 预共享密钥
AUTH_TOKEN = ""
...
if not AUTH_TOKEN:
    sys.exit("Missing CONTROL_SECRET")
...
class AgentHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if AUTH_TOKEN:
            ...
            signed_pairs = []
            for key in sorted(k for k in query.keys() if k not in ('t', 'sign')):
                for val in sorted(query.get(key, [])):
                    signed_pairs.append(f"{key}={val}")
            signed_query = "&".join(signed_pairs)
            signed_target = f"{req_path}?{signed_query}" if signed_query else req_path
            msg = f"{signed_target}:{req_t}".encode('utf-8')
        ...
        # 路由 8: OTA 远程热更新链路 (安全版永久熔断)
        elif req_path == '/trigger_ota':
            self.send_response(403)
            self.end_headers()
            self.wfile.write(b"403 Forbidden: OTA disabled in hardened build\n")
