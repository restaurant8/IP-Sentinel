...
is_forbidden_agent_host() {
    local host
    host=$(echo "$1" | tr -d '[]' | tr 'A-Z' 'a-z')
    ...
    case "$host" in
        localhost|0|0.0.0.0|::|::1|fc*|fd*|fe80:*) return 0 ;;
    esac

    if [[ "$host" =~ ^127\.|^10\.|^192\.168\.|^172\.(1[6-9]|2[0-9]|3[0-1])\.|^169\.254\.|^100\.(6[4-9]|[7-9][0-9]|1[0-1][0-9]|12[0-7])\. ]]; then
        return 0
    fi
...
generate_signed_url() {
    ...
    local target_port=$2
    local action_path=$3
    local control_secret=$4
    local signed_query=${5:-}
    local current_t=$(date +%s)

    if ! [[ "$control_secret" =~ ^[A-Fa-f0-9]{64,128}$ ]]; then
        return 1
    fi
    ...
    if is_forbidden_agent_host "$target_ip"; then
        return 1
    fi
    
    local signed_target="$action_path"
    if [ -n "$signed_query" ]; then
        signed_target="${action_path}?${signed_query}"
    fi
    local payload="${signed_target}:${current_t}"
    local signature=$(echo -n "$payload" | openssl dgst -sha256 -mac HMAC -macopt key:"$control_secret" | awk '{print $NF}')
    
    if [ -n "$signed_query" ]; then
        echo "https://${target_ip}:${target_port}${action_path}?${signed_query}&t=${current_t}&sign=${signature}"
    else
...
db_exec "ALTER TABLE nodes ADD COLUMN control_secret TEXT DEFAULT '';" 2>/dev/null
db_exec "UPDATE nodes SET enable_ota='false';" 2>/dev/null
        ...
        echo "$UPDATES" | jq -c '.result[]' | while read -r UPDATE; do
            ...
            if [[ "$TEXT" == *"#REGISTER#"* ]]; then
                ...
                if [ "$FIELD_COUNT" -ge 8 ]; then
                    IFS='|' read -r MAGIC RAW_REGION RAW_NODE RAW_IP RAW_PORT RAW_ALIAS RAW_OTA RAW_SECRET <<< "$REG_LINE"
                ...
                AGENT_OTA="false"
                CONTROL_SECRET=$(echo "$RAW_SECRET" | tr -cd 'a-fA-F0-9' | cut -c 1-128)
                
                # SSRF 拦截墙
                if is_forbidden_agent_host "$AGENT_IP"; then
                    send_msg "$CHAT_ID" "⛔ **安全拦截**：禁止注册内网或回环 IP，防止 SSRF 攻击渗透。"
                    continue
                fi
                
                if [ -z "$NODE_NAME" ] || [ -z "$AGENT_IP" ] || [ -z "$AGENT_PORT" ] || [ -z "$CHAT_ID" ] || ! [[ "$CONTROL_SECRET" =~ ^[A-Fa-f0-9]{64,128}$ ]]; then
                    send_msg "$CHAT_ID" "⛔ **安全拦截**：检测到非法注册载荷，请求已拒绝。"
                    continue
                fi

                db_exec "INSERT INTO nodes (chat_id, node_name, agent_ip, agent_port, last_seen, region, node_alias, enable_ota, control_secret) VALUES ('$CHAT_ID', '$NODE_NAME', '$AGENT_IP', '$AGENT_PORT', CURRENT_TIMESTAMP, '$AGENT_REGION', '$NODE_ALIAS', '$AGENT_OTA', '$CONTROL_SECRET') ON CONFLICT(chat_id, node_name) DO UPDATE SET agent_ip='$AGENT_IP', agent_port='$AGENT_PORT', last_seen=CURRENT_TIMESTAMP, region='$AGENT_REGION', node_alias='$NODE_ALIAS', enable_ota='false', control_secret='$CONTROL_SECRET';"
            ...
            case "$TEXT" in
                "/start"|"/menu")
                    REMOTE_VER=""
                    if [ "${ENABLE_REMOTE_VERSION_CHECK:-false}" == "true" ] && { ! [[ "$REPO_RAW_URL" =~ /(main|master)$ ]] || [ "${ALLOW_FLOATING_REMOTE:-false}" == "true" ]; }; then
                    ...
                    if [ -n "$REMOTE_VER" ]; then
                        if [ "$REMOTE_VER" != "$MASTER_VERSION" ]; then
                            VER_INFO="${VER_INFO}\n✨ **发现新版本**: \`v${REMOTE_VER}\` (安全版请通过 SSH 手动升级)"
                        else
                    ...
                    if [ "$IS_OFFICIAL_GATEWAY" != "true" ]; then
                        BTNS="[${BTN_MASTER_OTA}[{\"text\":\"🌍 进入全球雷达 (管理节点)\",\"callback_data\":\"list_nodes\"}], [{\"text\":\"🚀 唤醒全局巡逻\",\"callback_data\":\"all_run\"}, {\"text\":\"📊 获取全局简报\",\"callback_data\":\"all_reports\"}], [{\"text\":\"🌟 前往 GitHub 点亮星标\",\"url\":\"https://github.com/hotyue/IP-Sentinel\"}]]"
                    else
                ...
                "all_ota_confirm")
                    send_msg "$CHAT_ID" "⛔ 安全版已禁用全舰队 OTA。请通过 SSH 手动升级节点。"
                    ;;
                ...
                "master_ota_confirm")
                    send_msg "$CHAT_ID" "⛔ 安全版已禁用司令部 OTA。请通过 SSH 手动升级 Master。"
                    ;;
                ...
                "all_reports")
                    NODE_DATA=$(db_exec "SELECT node_name, agent_ip, agent_port, control_secret FROM nodes WHERE chat_id='$CHAT_ID';")
                    if [ -z "$NODE_DATA" ]; then
                        send_msg "$CHAT_ID" "⚠️ 您名下暂无在线节点。"
                    else
                        send_msg "$CHAT_ID" "📢 **司令部指令下达：正在召唤所有哨兵回传简报...**%0A*(为防止触发 TG 官方限流，简报将排队依次送达，请耐心等待)*"
                        echo "$NODE_DATA" | while IFS='|' read -r NNAME AIP APORT ASECRET; do
                            TARGET_URL=$(generate_signed_url "$AIP" "$APORT" "/trigger_report" "$ASECRET") || continue
                ...
                "all_run")
                    ...
                    if [ -z "$NODE_DATA" ]; then
                        ...
                        echo "$NODE_DATA" | while IFS='|' read -r NNAME AIP APORT ASECRET; do
                            TARGET_URL=$(generate_signed_url "$AIP" "$APORT" "/trigger_run" "$ASECRET") || continue
                            curl -k -s --connect-timeout 5 -m 15 "$TARGET_URL" > /dev/null &
                            sleep 0.2  
                        done
                    fi
                ...
                "/quality"|"/quality@"*)
                    ...
                    if [ -z "$TARGET_NODE" ]; then
                        ...
                        AGENT_INFO=$(db_exec "SELECT agent_ip, agent_port, control_secret FROM nodes WHERE chat_id='$CHAT_ID' AND node_name='$TARGET_NODE' LIMIT 1;")
                        AGENT_IP=$(echo "$AGENT_INFO" | cut -d'|' -f1)
                        AGENT_PORT=$(echo "$AGENT_INFO" | cut -d'|' -f2)
                        AGENT_SECRET=$(echo "$AGENT_INFO" | cut -d'|' -f3)

                        if [ -n "$AGENT_IP" ] && [ -n "$AGENT_PORT" ]; then
                            send_msg "$CHAT_ID" "⏳ 正在向 \`$TARGET_NODE\` ($AGENT_IP) 下发 [quality] 指令，请稍候..."
                            
                            TARGET_URL=$(generate_signed_url "$AGENT_IP" "$AGENT_PORT" "/trigger_quality" "$AGENT_SECRET" || true)
                            if [ -z "$TARGET_URL" ]; then
                                send_msg "$CHAT_ID" "⛔ 该节点缺少高强度控制密钥，请重新注册节点。"
                                continue
                            fi
                ...
                toggle:*)
                    ...
                    CHAT_ID=$(echo "$CHAT_ID" | tr -cd '0-9-')
                    MOD_NAME=$(echo "$MOD_NAME" | tr -cd 'a-z')
                    TARGET_NODE=$(echo "$TARGET_NODE" | tr -cd 'a-zA-Z0-9_.-')
                    TARGET_STATE=$(echo "$TARGET_STATE" | tr -cd 'a-z')
                    if [[ "$MOD_NAME" != "google" && "$MOD_NAME" != "trust" ]] || [[ "$TARGET_STATE" != "true" && "$TARGET_STATE" != "false" ]]; then
                        send_msg "$CHAT_ID" "⛔ **安全拦截**：非法模块切换请求已拒绝。"
                        continue
                    fi
                    ...
                    if [ -n "$AGENT_IP" ] && [ -n "$AGENT_PORT" ]; then
                        TARGET_URL=$(generate_signed_url "$AGENT_IP" "$AGENT_PORT" "/trigger_toggle" "$AGENT_SECRET" "mod=${MOD_NAME}&state=${TARGET_STATE}" || true)
                        if [ -z "$TARGET_URL" ]; then
                            send_msg "$CHAT_ID" "⛔ 该节点缺少高强度控制密钥，请重新注册节点。"
                            continue
                        fi
                ...
                do_rename:*)
                    ...
                    CHAT_ID=$(echo "$CHAT_ID" | tr -cd '0-9-')
                    TARGET_NODE=$(echo "$TARGET_NODE" | tr -cd 'a-zA-Z0-9_.-')
                    NEW_ALIAS=$(echo "$NEW_ALIAS" | sed 's/_/-/g' | tr -d '"'\''\`\$\|&;<>\n\r:' | cut -c 1-30)
                    if [ -z "$TARGET_NODE" ] || [ -z "$NEW_ALIAS" ]; then
                        send_msg "$CHAT_ID" "⛔ **安全拦截**：非法重命名请求已拒绝。"
                        continue
                    fi
                    ...
                    if [ -n "$AGENT_IP" ] && [ -n "$AGENT_PORT" ]; then
                        ...
                        TARGET_URL=$(generate_signed_url "$AGENT_IP" "$AGENT_PORT" "/trigger_rename" "$AGENT_SECRET" "b64=${ALIAS_B64}" || true)
                        if [ -z "$TARGET_URL" ]; then
                            send_msg "$CHAT_ID" "⛔ 该节点缺少高强度控制密钥，请重新注册节点。"
                            continue
                        fi
                ...
                ota_confirm:*)
                    TARGET_NODE=$(echo "${TEXT#*:}" | tr -cd 'a-zA-Z0-9_.-')
                    send_msg "$CHAT_ID" "⛔ 安全版已禁用节点 OTA。请通过 SSH 手动升级 \`$TARGET_NODE\`。"
                    ;;
                ...
                google:*|trust:*|run:*|report:*|log:*|quality:*)
                    ...
                    if [ -n "$AGENT_IP" ] && [ -n "$AGENT_PORT" ]; then
                        ...
                        TARGET_URL=$(generate_signed_url "$AGENT_IP" "$AGENT_PORT" "/trigger_${ACTION_TYPE}" "$AGENT_SECRET" || true)
                        if [ -z "$TARGET_URL" ]; then
                            send_msg "$CHAT_ID" "⛔ 该节点缺少高强度控制密钥，请重新注册节点。"
                            continue
                        fi
